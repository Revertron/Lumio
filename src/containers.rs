...
impl Frame {
    pub fn new(rect: Rect<i32>, width: Dimension, height: Dimension) -> Frame {
        ...
        Frame {
            ...
            views: Vec::new(),
            layout: Box::new(LinearLayout::default()),
            background_image: RefCell::new(None)
        }
    }

    /// Replaces the layout strategy that positions this frame's children.
    /// The default is [`LinearLayout`].
    ...
    /// Sets the flow direction. Only applies while the layout is the default
    /// [`LinearLayout`]; ignored for custom layouts.
    fn set_direction(&mut self, direction: Direction) {
        if let Some(l) = self.layout.downcast_mut::<LinearLayout>() {
            l.direction = direction;
        }
    }

    /// Enables row wrapping. Only applies while the layout is the default
    /// [`LinearLayout`]; ignored for custom layouts.
    fn set_breaking(&mut self, breaking: bool) {
        if let Some(l) = self.layout.downcast_mut::<LinearLayout>() {
            l.breaking = breaking;
        }
    }
...
impl View for Frame {
    ...
    fn layout_content(&mut self, x: i32, y: i32, width: i32, height: i32, typeface: &Typeface, scale: f64) -> Rect<i32> {
        ...
        self.layout.arrange(&self.views, self.get_bounds(), new_width, new_height, &padding, &typeface, scale);
    ...
    fn on_key_down(&self, ui: &mut UI, virtual_key_code: Option<VirtualKeyCode>, scancode: KeyScancode, state: ModifiersState) -> bool {
        ...
        if let Some(code) = virtual_key_code {
            ...
            if code == VirtualKeyCode::Right && direction == Direction::Horizontal {
                if self.focus_next() {
                    return true;
                }
            }
            if code == VirtualKeyCode::Left && direction == Direction::Horizontal {
                if self.focus_prev() {
                    return true;
                }
            }
            if code == VirtualKeyCode::Up && direction == Direction::Vertical {
                if self.focus_prev() {
                    return true;
                }
            }
            if code == VirtualKeyCode::Down && direction == Direction::Vertical {
                if self.focus_next() {
                    return true;
                }
            }
