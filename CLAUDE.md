...
- **Window loop**: one backend-neutral winit `ApplicationHandler` in **`src/window/`** (15ms tick; multi-window + app-modal dialogs), behind the neutral `lumio::run(ui, WindowConfig)` launcher (`src/app.rs`). The per-window paint sits behind a `RenderSurface` trait, cfg-selected per backend: `GlSurface` (`window/surface_gl.rs`, glutin context + `speedy2d::GLRenderer` → `RendererGL`) or `SoftwareSurface` (`window/surface_software.rs`, tiny-skia → softbuffer). winit→Lumio input conversions live in `window/input_winit.rs`. Input/event types are backend-neutral (`src/input/`). See docs/unified_window_loop.md for the design.

- **Renderer System** (`src/themes/`): The rendering abstraction seam is the `Renderer` trait. `RendererGL` (GL) and `RendererSoftware` (tiny-skia) are the two implementations, cfg-selected by backend. Handles rendering of different view states (focused, hovered, pressed, etc.). Text shaping is abstracted behind `crate::text` (speedy2d vs fontdue). (The visual *look* — palette colors/dimensions/typefaces — is carried separately by `Palette`; see `src/drawing/palette.rs` and docs/theming.md.)
